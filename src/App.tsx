...
export default function App(){
  ...
  const trackTouchStart=(e:ReactPointerEvent<HTMLDivElement>)=>{if(e.pointerType!=="touch"||(e.target as HTMLElement).closest(".zoom"))return;e.currentTarget.setPointerCapture(e.pointerId);touches.current.set(e.pointerId,{x:e.clientX,y:e.clientY});if(touches.current.size!==2)return;const [a,b]=Array.from(touches.current.values()),r=canvas.current?.getBoundingClientRect();if(!r)return;const current=view.current,mx=(a.x+b.x)/2-r.left,my=(a.y+b.y)/2-r.top;pinch.current={distance:Math.max(1,Math.hypot(b.x-a.x,b.y-a.y)),zoom:current.zoom,worldX:(mx-current.pan.x)/current.zoom,worldY:(my-current.pan.y)/current.zoom};moving.current=null};
